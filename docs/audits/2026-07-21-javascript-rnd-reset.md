...
- Expo SDK 57 dependency compatibility is checked against Expo's bundled version map.
- `npm audit --audit-level=high` passes with zero high or critical findings.
- npm currently reports ten **moderate** transitive findings in Expo native-build tooling through `xcode`/`uuid`. npm's proposed automatic remedy is an unsafe downgrade to Expo 46, so no forced fix is applied. These findings remain documented and must be re-evaluated before any real-user build.
