...
struct ProximitySettingsView: View {
        ...
        List {
            ...
            Section("Compatible genders") {
                ForEach(GenderIdentity.allCases.filter { $0 != .preferNotToSay }) { gender in
                    Toggle(gender.rawValue, isOn: setBinding(gender, in: \ProximityPreferences.compatibleGenders))
                }
            }
    ...
    private func setBinding(
        _ gender: GenderIdentity,
        in keyPath: WritableKeyPath<ProximityPreferences, Set<GenderIdentity>>
    ) -> Binding<Bool> {
        Binding(
            get: { model.proximityPreferences[keyPath: keyPath].contains(gender) },
            set: { enabled in
                var copy = model.proximityPreferences
                if enabled {
                    copy[keyPath: keyPath].insert(gender)
                } else {
                    copy[keyPath: keyPath].remove(gender)
                }
                ...
                VStack(spacing: 12) {
                    Map(
                        ...
                        if share.mode.isPrecise {
                            Marker(profile.displayName, coordinate: CLLocationCoordinate2D(latitude: share.latitude, longitude: share.longitude))
                        } else {
                            MapCircle(
                                center: CLLocationCoordinate2D(latitude: share.latitude, longitude: share.longitude),
                                radius: 2_000
